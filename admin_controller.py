...
# Admin side of things. There's no registration for admins, they just
# log in with a fixed shared password. The service methods (group_by_grade,
# group_by_pass_fail, etc.) are also called by the GUI, so keep them
# free of any CLI prints.
...
class AdminController:
    ...
    def check_login_credential(password):
        # Admins only need a password, no email.
    ...
    def group_by_grade(self):
        # Buckets students under their overall grade. Anyone with no
        # subjects is skipped because their grade isn't meaningful.
        groups = {"HD": [],
                    "D": [],
                    "C": [],
                    "P": [],
        ...
        # Drop empty grade buckets so the output is cleaner.
        non_empty_groups = {}
    ...
    def group_by_pass_fail(self):
        # Pass if the average mark is 50 or higher. Students with no
        # enrolments are left out for the same reason as above.
        result = {"PASS": [],
                                   "FAIL": []}
        for student in self._db.read_all_students():
            if not student.subjects:
    ...
    # Below here is the CLI-only menu stuff.
